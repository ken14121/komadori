...
- 🔄 **LMS連携** — Open LMS(Moodle)から課題の締切を自動取り込み。アプリを開くたびに最新化
...
学校の Open LMS(Moodle ベース)から、課題の締切を自動で取り込めます。**スクショもAI解析も不要・API利用料もゼロ**です。Moodle 公式の Web Services API を使います。

**この機能だけは Vercel へのデプロイが必要です。** Moodle は CORS ヘッダーを返さないため、ブラウザから直接呼べません。`api/lms.js`(中継関数)を挟んで解決しています。

トークンの取得(パスワードは使いません):

1. ブラウザで LMS にログイン
2. 右上の自分の名前 → **環境設定** → **セキュリティキー**
3. **Moodle モバイルウェブサービス** のキーをコピー
4. コマドリの「設定 → LMS連携」に貼り付け → **接続テスト**

以降、アプリを開くたびに(30分に1回まで)課題が自動で最新化されます。取り込んだ課題の**完了マークとメモは再同期しても保持**され、締切変更だけが反映されます。トークンはこの端末のブラウザ内にのみ保存され、LMS 側からいつでも無効化できます。

> 対象 LMS を変える場合は、Vercel の環境変数 `LMS_BASE` を設定してください(既定: `https://lms-tokyo.iput.ac.jp`)。
...
- **APIキー / LMSトークン** — ブラウザの localStorage のみ。サーバーには一切送らない・保存しない
- **`api/lms.js`** — トークンを受け取って LMS に中継するだけで、保存もログ出力もしない。転送先は `LMS_BASE` の1ホストに固定(踏み台にできない)。呼べる Moodle 関数は読み取り専用の3つのみ
- **パスワードは扱わない** — LMS のログインは中継しない(総当たりの経路を作らないため)。トークンは利用者が LMS 画面から取得する
...
js/lms.js             LMS連携(Moodle Web Services API)
js/settings.js        設定画面
